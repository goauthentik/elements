Note: Many of the elements in `@goauthentik/foundation` are taken directly from Patternfly Elements,
but they have been updated to be compatible with Patternfly 5 where applicable. We have also built
our own copy of Patternfly 5, using the toolkit provided to provide our own prefix.

The rest of this document includes the rest of the original README.

# PatternFly Elements Button

`<pf-button>` is a web component wrapper around a standard HTML `<button>` element. `<pf-button>` is
heavily influenced by the findings in this post: [Custom elements, shadow DOM and implicit form
submission](https://www.hjorthhansen.dev/shadow-dom-and-forms/). You can expect `<pf-button>` to
work like a normal `HTMLButtonElement`.
